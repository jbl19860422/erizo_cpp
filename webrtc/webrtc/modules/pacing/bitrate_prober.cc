...
int ComputeDeltaFromBitrate(size_t probe_size, uint32_t bitrate_bps) {
  LICODE_RTC_CHECK_GT(bitrate_bps, 0);
...
void BitrateProber::CreateProbeCluster(int bitrate_bps, int num_probes) {
  LICODE_RTC_DCHECK(probing_state_ != ProbingState::kDisabled);
...
int BitrateProber::CurrentClusterId() const {
  LICODE_RTC_DCHECK(!clusters_.empty());
  LICODE_RTC_DCHECK(ProbingState::kActive == probing_state_);
...
void BitrateProber::ProbeSent(int64_t now_ms, size_t bytes) {
  LICODE_RTC_DCHECK(probing_state_ == ProbingState::kActive);
  LICODE_RTC_DCHECK_GT(bytes, 0);
