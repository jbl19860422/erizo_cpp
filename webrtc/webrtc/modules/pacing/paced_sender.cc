...
class PacketQueue {
 public:
  ...
  void FinalizePop(const Packet& packet) {
    ...
    LICODE_RTC_DCHECK_EQ(packet_list_.size(), prio_queue_.size());
    if (packet_list_.empty())
      LICODE_RTC_DCHECK_EQ(0, queue_time_sum_);
  }
  ...
  void UpdateQueueTime(int64_t timestamp_ms) {
    LICODE_RTC_DCHECK_GE(timestamp_ms, time_last_updated_);
 ...
 private:
  ...
  void RemoveFromDupeSet(const Packet& packet) {
    SsrcSeqNoMap::iterator it = dupe_map_.find(packet.ssrc);
    LICODE_RTC_DCHECK(it != dupe_map_.end());
...
void PacedSender::SetProbingEnabled(bool enabled) {
  LICODE_RTC_CHECK_EQ(0, packet_counter_);
  CriticalSectionScoped cs(critsect_.get());
...
void PacedSender::InsertPacket(RtpPacketSender::Priority priority,
                               ...
                               bool retransmission) {
  CriticalSectionScoped cs(critsect_.get());
  LICODE_RTC_DCHECK(estimated_bitrate_bps_ > 0)
...
int64_t PacedSender::ExpectedQueueTimeMs() const {
  CriticalSectionScoped cs(critsect_.get());
  LICODE_RTC_DCHECK_GT(pacing_bitrate_kbps_, 0);
