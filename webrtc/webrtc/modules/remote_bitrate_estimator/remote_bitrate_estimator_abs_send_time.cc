...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketFeedbackVector(
  ...
  LICODE_RTC_DCHECK(network_thread_.CalledOnValidThread());
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketInfo(
    ...
    size_t payload_size,
    uint32_t ssrc) {
  LICODE_RTC_CHECK(send_time_24bits < (1ul << 24));
  if (!uma_recorded_) {
  ...
  std::vector<uint32_t> ssrcs;
  {
    ...
    LICODE_RTC_DCHECK(inter_arrival_.get());
    LICODE_RTC_DCHECK(estimator_.get());
    ssrcs_[ssrc] = now_ms;
...
bool RemoteBitrateEstimatorAbsSendTime::LatestEstimate(
  ...
  // thread.
  LICODE_RTC_DCHECK(ssrcs);
  LICODE_RTC_DCHECK(bitrate_bps);
  rtc::CritScope lock(&crit_);
