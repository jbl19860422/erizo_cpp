...
bool AudioLevel::Write(uint8_t* data,
                       bool voice_activity,
                       uint8_t audio_level) {
  LICODE_RTC_CHECK_LE(audio_level, 0x7f);
...
bool TransmissionOffset::Write(uint8_t* data, int32_t rtp_time) {
  LICODE_RTC_DCHECK_LE(rtp_time, 0x00ffffff);
...
bool PlayoutDelayLimits::Parse(const uint8_t* data,
  ...
  LICODE_RTC_DCHECK(playout_delay);
...
bool PlayoutDelayLimits::Write(uint8_t* data,
  ...
  LICODE_RTC_DCHECK_LE(0, playout_delay.min_ms);
  LICODE_RTC_DCHECK_LE(playout_delay.min_ms, playout_delay.max_ms);
  LICODE_RTC_DCHECK_LE(playout_delay.max_ms, kMaxMs);
