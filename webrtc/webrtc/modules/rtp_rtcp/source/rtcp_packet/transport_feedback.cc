 ...
 public:
  ...
  void WriteTo(uint8_t* buffer) const override {
    ...
    for (int i = 0; i < kSymbolsInFirstByte; ++i) {
      ...
      LICODE_RTC_DCHECK_LE(encoded_symbol, 1);
...
class RunLengthChunk : public TransportFeedback::PacketStatusChunk {
 public:
  RunLengthChunk(TransportFeedback::StatusSymbol symbol, size_t size)
    ...
    LICODE_RTC_DCHECK_LE(size, 0x1FFFu);
  }
  ...
  static RunLengthChunk* ParseFrom(const uint8_t* buffer) {
    LICODE_RTC_DCHECK_EQ(0, buffer[0] & 0x80);
...
void TransportFeedback::SetBase(uint16_t base_sequence,
  ...
  LICODE_RTC_DCHECK_EQ(-1, base_seq_);
  LICODE_RTC_DCHECK_NE(-1, ref_timestamp_us);
...
bool TransportFeedback::AddReceivedPacket(uint16_t sequence_number,
                                          int64_t timestamp) {
  LICODE_RTC_DCHECK_NE(-1, base_seq_);
...
bool TransportFeedback::Encode(StatusSymbol symbol) {
  ...
  LICODE_RTC_DCHECK_GE(delta_size, 0);
...
void TransportFeedback::EmitRunLengthChunk() {
  LICODE_RTC_DCHECK_GE(first_symbol_cardinality_, symbol_vec_.size());
...
bool TransportFeedback::Create(uint8_t* packet,
  ...
  *position += kCommonFeedbackLength;
  ...
  LICODE_RTC_DCHECK_LE(base_seq_, 0xFFFF);
  ByteWriter<uint16_t>::WriteBigEndian(&packet[*position], base_seq_);
  *position += 2;

  int64_t status_count = last_seq_ - base_seq_ + 1;
  LICODE_RTC_DCHECK_LE(status_count, 0xFFFF);
  ...
  *position += 3;
  ...
  LICODE_RTC_DCHECK_EQ(*position, position_end);
  return true;
}
...
bool TransportFeedback::Parse(const CommonHeader& packet) {
  LICODE_RTC_DCHECK_EQ(packet.type(), kPacketType);
  LICODE_RTC_DCHECK_EQ(packet.fmt(), kFeedbackMessageType);
  ...
  LICODE_RTC_DCHECK(status_chunks_.empty()) << "Parse expects object to be new.";
  ...
  LICODE_RTC_DCHECK_EQ(num_packets, symbols.size());
  ...
  LICODE_RTC_DCHECK_LE(index, end_index);
