...
rtc::Buffer RtcpPacket::Build() const {
  ...
  LICODE_RTC_DCHECK(created) << "Invalid packet is not supported.";
  LICODE_RTC_DCHECK_EQ(length, packet.size())
...
bool RtcpPacket::OnBufferFull(uint8_t* packet,
  ...
  if (*index == 0)
    return false;
  LICODE_RTC_DCHECK(callback) << "Fragmentation not supported.";
...
size_t RtcpPacket::HeaderLength() const {
  ...
  LICODE_RTC_DCHECK_GT(length_in_bytes, 0);
  LICODE_RTC_DCHECK_EQ(length_in_bytes % 4, 0) << "Padding not supported";
...
void RtcpPacket::CreateHeader(
    ...
    uint8_t* buffer,
    size_t* pos) {
  LICODE_RTC_DCHECK_LE(length, 0xffffU);
  LICODE_RTC_DCHECK_LE(count_or_format, 0x1f);
