...
#ifndef LICODE_WEBRTC_MODULES_RTP_RTCP_SOURCE_RTP_HEADER_EXTENSION_H_
#define LICODE_WEBRTC_MODULES_RTP_RTCP_SOURCE_RTP_HEADER_EXTENSION_H_
...
class RtpHeaderExtensionMap {
 public:
  ...
  RTPExtensionType GetType(uint8_t id) const {
    LICODE_RTC_DCHECK_GE(id, kMinId);
    LICODE_RTC_DCHECK_LE(id, kMaxId);
    return types_[id];
  }
  // Return kInvalidId if not found.
  uint8_t GetId(RTPExtensionType type) const {
    LICODE_RTC_DCHECK_GT(type, kRtpExtensionNone);
    LICODE_RTC_DCHECK_LT(type, kRtpExtensionNumberOfExtensions);
    return ids_[type];
  }
