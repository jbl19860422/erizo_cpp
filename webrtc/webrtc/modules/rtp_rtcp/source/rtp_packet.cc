...
Packet::Packet(const ExtensionManager* extensions, size_t capacity)
  ...
  LICODE_RTC_DCHECK_GE(capacity, kFixedHeaderSize);
  Clear();
}
...
bool Packet::Parse(const uint8_t* buffer, size_t buffer_size) {
  ...
  LICODE_RTC_DCHECK_EQ(size(), buffer_size);
  return true;
}
...
bool Packet::Marker() const {
  LICODE_RTC_DCHECK_EQ(marker_, (data()[1] & 0x80) != 0);
  return marker_;
}

uint8_t Packet::PayloadType() const {
  LICODE_RTC_DCHECK_EQ(payload_type_, data()[1] & 0x7f);
  return payload_type_;
}

uint16_t Packet::SequenceNumber() const {
  LICODE_RTC_DCHECK_EQ(sequence_number_,
                ByteReader<uint16_t>::ReadBigEndian(data() + 2));
  return sequence_number_;
}

uint32_t Packet::Timestamp() const {
  LICODE_RTC_DCHECK_EQ(timestamp_, ByteReader<uint32_t>::ReadBigEndian(data() + 4));
  return timestamp_;
}

uint32_t Packet::Ssrc() const {
  LICODE_RTC_DCHECK_EQ(ssrc_, ByteReader<uint32_t>::ReadBigEndian(data() + 8));
  return ssrc_;
}

std::vector<uint32_t> Packet::Csrcs() const {
  size_t num_csrc = data()[0] & 0x0F;
  LICODE_RTC_DCHECK_GE(capacity(), kFixedHeaderSize + num_csrc * 4);
...
size_t Packet::size() const {
  ...
  LICODE_RTC_DCHECK_EQ(buffer_.size(), ret);
  return ret;
}
...
void Packet::CopyHeaderFrom(const Packet& packet) {
  LICODE_RTC_DCHECK_GE(capacity(), packet.headers_size());
...
void Packet::SetPayloadType(uint8_t payload_type) {
  LICODE_RTC_DCHECK_LE(payload_type, 0x7Fu);
...
void Packet::SetCsrcs(const std::vector<uint32_t>& csrcs) {
  LICODE_RTC_DCHECK_EQ(num_extensions_, 0);
  LICODE_RTC_DCHECK_EQ(payload_size_, 0);
  LICODE_RTC_DCHECK_EQ(padding_size_, 0);
  LICODE_RTC_DCHECK_LE(csrcs.size(), 0x0fu);
  LICODE_RTC_DCHECK_LE(kFixedHeaderSize + 4 * csrcs.size(), capacity());
...
void Packet::SetPayloadSize(size_t size_bytes) {
  LICODE_RTC_DCHECK_EQ(padding_size_, 0);
  LICODE_RTC_DCHECK_LE(size_bytes, payload_size_);
  payload_size_ = size_bytes;
  buffer_.SetSize(payload_offset_ + payload_size_);
}

bool Packet::SetPadding(uint8_t size_bytes, Random* random) {
  LICODE_RTC_DCHECK(random);
...
bool Packet::AllocateExtension(ExtensionType type,
  ...
  LICODE_RTC_DCHECK_GT(length, 0);
  LICODE_RTC_DCHECK_LE(length, 16);
  ...
  if (num_extensions_ == 0) {
    LICODE_RTC_DCHECK_EQ(payload_offset_, kFixedHeaderSize + (num_csrc * 4));
    LICODE_RTC_DCHECK_EQ(extensions_size_, 0);
  ...
  LICODE_RTC_DCHECK(num_extensions_ < kMaxExtensionHeaders);
