...
void RtpPacketizerH264::SetPayloadData(
  ...
  LICODE_RTC_DCHECK(packets_.empty());
  LICODE_RTC_DCHECK(input_fragments_.empty());
  LICODE_RTC_DCHECK(fragmentation);
...
void RtpPacketizerH264::PacketizeFuA(size_t fragment_index) {
  ...
  LICODE_RTC_CHECK_EQ(0, fragment_length);
}
...
size_t RtpPacketizerH264::PacketizeStapA(size_t fragment_index) {
  ...
  const Fragment* fragment = &input_fragments_[fragment_index];
  LICODE_RTC_CHECK_GE(payload_size_left, fragment->length);
  while (payload_size_left >= fragment->length + fragment_headers_length) {
    LICODE_RTC_CHECK_GT(fragment->length, 0);
...
void RtpPacketizerH264::PacketizeSingleNalu(size_t fragment_index) {
  ...
  LICODE_RTC_CHECK_GT(fragment->length, 0u);
...
bool RtpPacketizerH264::NextPacket(RtpPacketToSend* rtp_packet,
                                   bool* last_packet) {
  LICODE_RTC_DCHECK(rtp_packet);
  LICODE_RTC_DCHECK(last_packet);
  if (packets_.empty()) {
    *last_packet = true;
    return false;
  }
  ...
  if (packet.first_fragment && packet.last_fragment) {
    ...
    LICODE_RTC_CHECK_EQ(H264PacketizationMode::NonInterleaved, packetization_mode_);
    NextAggregatePacket(rtp_packet);
  } else {
    LICODE_RTC_CHECK_EQ(H264PacketizationMode::NonInterleaved, packetization_mode_);
    NextFragmentPacket(rtp_packet);
  }
  LICODE_RTC_DCHECK_LE(rtp_packet->payload_size(), max_payload_len_);
...
void RtpPacketizerH264::NextAggregatePacket(RtpPacketToSend* rtp_packet) {
  ...
  LICODE_RTC_DCHECK(buffer);
  PacketUnit* packet = &packets_.front();
  LICODE_RTC_CHECK(packet->first_fragment);
  // STAP-A NALU header.
  ...
  LICODE_RTC_CHECK(packet->last_fragment);
...
bool RtpDepacketizerH264::Parse(ParsedPayload* parsed_payload,
  ...
  LICODE_RTC_CHECK(parsed_payload != nullptr);
