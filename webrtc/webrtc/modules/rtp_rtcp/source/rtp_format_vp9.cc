...
size_t RefIndicesLength(const RTPVideoHeaderVP9& hdr) {
  ...
  LICODE_RTC_DCHECK_GT(hdr.num_ref_pics, 0U);
  LICODE_RTC_DCHECK_LE(hdr.num_ref_pics, kMaxVp9RefPics);
...
size_t SsDataLength(const RTPVideoHeaderVP9& hdr) {
  ...
  LICODE_RTC_DCHECK_GT(hdr.num_spatial_layers, 0U);
  LICODE_RTC_DCHECK_LE(hdr.num_spatial_layers, kMaxVp9NumberOfSpatialLayers);
  LICODE_RTC_DCHECK_LE(hdr.gof.num_frames_in_gof, kMaxVp9FramesInGof);
  size_t length = 1;                           // V
  ...
  for (size_t i = 0; i < hdr.gof.num_frames_in_gof; ++i) {
    LICODE_RTC_DCHECK_LE(hdr.gof.num_ref_pics[i], kMaxVp9RefPics);
...
bool WriteSsData(const RTPVideoHeaderVP9& vp9, rtc::BitBufferWriter* writer) {
  LICODE_RTC_DCHECK_GT(vp9.num_spatial_layers, 0U);
  LICODE_RTC_DCHECK_LE(vp9.num_spatial_layers, kMaxVp9NumberOfSpatialLayers);
  LICODE_RTC_DCHECK_LE(vp9.gof.num_frames_in_gof, kMaxVp9FramesInGof);
