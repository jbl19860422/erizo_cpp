...
int ForwardErrorCorrection::EncodeFec(const PacketList& media_packets,
  ...
  LICODE_RTC_DCHECK_GT(num_media_packets, 0);
  LICODE_RTC_DCHECK_GE(num_important_packets, 0);
  LICODE_RTC_DCHECK_LE(num_important_packets, num_media_packets);
  LICODE_RTC_DCHECK(fec_packets->empty());
  ...
  for (const auto& media_packet : media_packets) {
    LICODE_RTC_DCHECK(media_packet);
...
int ForwardErrorCorrection::NumFecPackets(int num_media_packets,
  ...
  LICODE_RTC_DCHECK_LE(num_fec_packets, num_media_packets);
...
void ForwardErrorCorrection::GenerateFecPayloads(
  ...
  LICODE_RTC_DCHECK(!media_packets.empty());
  for (size_t i = 0; i < num_fec_packets; ++i) {
    ...
    LICODE_RTC_DCHECK_GT(fec_packet->length, 0)
...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  if (fec_packet->protected_packets.empty()) {
    ...
    LICODE_RTC_DCHECK_LE(received_fec_packets_.size(), max_fec_packets);
  }
}
...
void ForwardErrorCorrection::InsertPackets(
  ...
  LICODE_RTC_DCHECK(received_packets->empty());
...
void ForwardErrorCorrection::XorPayloads(const Packet& src,
                                         ...
                                         Packet* dst) {
  // XOR the payload.
  LICODE_RTC_DCHECK_LE(kRtpHeaderSize + payload_length, sizeof(src.data));
  LICODE_RTC_DCHECK_LE(dst_offset + payload_length, sizeof(dst->data));
...
void ForwardErrorCorrection::DiscardOldRecoveredPackets(
  ...
  LICODE_RTC_DCHECK_LE(recovered_packets->size(), max_media_packets);
}
