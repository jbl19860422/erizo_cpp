...
class RtcHistogram {
 public:
  RtcHistogram(const std::string& name, int min, int max, int bucket_count)
    ...
    LICODE_RTC_DCHECK_GT(bucket_count, 0);
  }
...
void Enable() {
  LICODE_RTC_DCHECK(g_rtc_histogram_map == nullptr);
#if LICODE_RTC_DCHECK_IS_ON
  LICODE_RTC_DCHECK_EQ(0, rtc::AtomicOps::AcquireLoad(&g_rtc_histogram_called));
#endif
  CreateMap();
}
