...
uint8_t LowestBits(uint8_t byte, size_t bit_count) {
  LICODE_RTC_DCHECK_LE(bit_count, 8);
...
uint8_t WritePartialByte(uint8_t source,
  ...
  LICODE_RTC_DCHECK(target_bit_offset < 8);
  LICODE_RTC_DCHECK(source_bit_count < 9);
  LICODE_RTC_DCHECK(source_bit_count <= (8 - target_bit_offset));
...
BitBuffer::BitBuffer(const uint8_t* bytes, size_t byte_count)
  ...
  LICODE_RTC_DCHECK(static_cast<uint64_t>(byte_count_) <=
...
bool BitBuffer::ReadUInt8(uint8_t* val) {
  ...
  LICODE_RTC_DCHECK(bit_val <= std::numeric_limits<uint8_t>::max());
...
bool BitBuffer::ReadUInt16(uint16_t* val) {
  ...
  LICODE_RTC_DCHECK(bit_val <= std::numeric_limits<uint16_t>::max());
...
bool BitBuffer::ReadExponentialGolomb(uint32_t* val) {
  ...
  LICODE_RTC_DCHECK(!PeekBits(&peeked_bit, 1) || peeked_bit == 1);
  ...
  if (value_bit_count > 32 || !ReadBits(val, value_bit_count)) {
    LICODE_RTC_CHECK(Seek(original_byte_offset, original_bit_offset));
    return false;
  }
  *val -= 1;
  return true;
...
void BitBuffer::GetCurrentOffset(
  ...
  LICODE_RTC_CHECK(out_byte_offset != NULL);
  LICODE_RTC_CHECK(out_bit_offset != NULL);
