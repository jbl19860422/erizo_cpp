...
void RateStatistics::Update(size_t count, int64_t now_ms) {
  ...
  LICODE_RTC_DCHECK_LT(now_offset, max_window_size_ms_);
  uint32_t index = oldest_index_ + now_offset;
  if (index >= max_window_size_ms_)
    index -= max_window_size_ms_;
  buckets_.get()[index].sum += count;
  ++buckets_.get()[index].samples;
...
void RateStatistics::EraseOld(int64_t now_ms) {
  ...
  while (num_samples_ > 0 && oldest_time_ < new_oldest_time) {
    const Bucket& oldest_bucket = buckets_.get()[oldest_index_];
    LICODE_RTC_DCHECK_GE(accumulated_count_, oldest_bucket.sum);
    LICODE_RTC_DCHECK_GE(num_samples_, oldest_bucket.samples);
    accumulated_count_ -= oldest_bucket.sum;
    num_samples_ -= oldest_bucket.samples;
    buckets_.get()[oldest_index_] = Bucket();
