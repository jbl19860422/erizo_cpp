...
#ifndef LICODE_WEBRTC_BASE_ARRAY_VIEW_H_
#define LICODE_WEBRTC_BASE_ARRAY_VIEW_H_
...
class ArrayView final {
 public:
  ...
  T& operator[](size_t idx) const {
    LICODE_RTC_DCHECK_LT(idx, size_);
    LICODE_RTC_DCHECK(data_);  // Follows from size_ > idx and the class invariant.
    return data_[idx];
  }
 ...
 private:
  ...
  void CheckInvariant() const { LICODE_RTC_DCHECK_EQ(!data_, size_ == 0); }
  T* data_;
  size_t size_;
};
