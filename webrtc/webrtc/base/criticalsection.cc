...
#else
#if defined(WEBRTC_MAC) && !LICODE_USE_NATIVE_MUTEX_ON_MAC
  lock_queue_ = 0;
  owning_thread_ = 0;
  recursion_ = 0;
...
#endif
  LICODE_CS_DEBUG_CODE(thread_ = 0);
  LICODE_CS_DEBUG_CODE(recursion_count_ = 0);
#endif
}
...
void CriticalSection::Enter() const EXCLUSIVE_LOCK_FUNCTION() {
  ...
  if (!have_lock && AtomicOps::Increment(&lock_queue_) > 1) {
    ...
    // have succeeded.
    LICODE_RTC_DCHECK(!IsThreadRefEqual(owning_thread_, self));
    // Wait for the lock to become available.
    dispatch_semaphore_wait(semaphore_, DISPATCH_TIME_FOREVER);
    LICODE_RTC_DCHECK(owning_thread_ == 0);
    LICODE_RTC_DCHECK(!recursion_);
  }
...
#if LICODE_CS_DEBUG_CHECKS
  if (!recursion_count_) {
    LICODE_RTC_DCHECK(!thread_);
    thread_ = CurrentThreadRef();
  } else {
    LICODE_RTC_DCHECK(CurrentThreadIsOwner());
  }
  ++recursion_count_;
#endif
#endif
...
#if LICODE_CS_DEBUG_CHECKS
  --recursion_count_;
  LICODE_RTC_DCHECK(recursion_count_ >= 0);
  if (!recursion_count_)
    thread_ = 0;
#endif
#if defined(WEBRTC_MAC) && !LICODE_USE_NATIVE_MUTEX_ON_MAC
  LICODE_RTC_DCHECK(IsThreadRefEqual(owning_thread_, CurrentThreadRef()));
  LICODE_RTC_DCHECK_GE(recursion_, 0);
  --recursion_;
  if (!recursion_)
    owning_thread_ = 0;
...
TryCritScope::TryCritScope(const CriticalSection* cs)
  ...
  LICODE_CS_DEBUG_CODE(lock_was_called_ = false);
}

TryCritScope::~TryCritScope() {
  LICODE_CS_DEBUG_CODE(LICODE_RTC_DCHECK(lock_was_called_));
  if (locked_)
    cs_->Leave();
}

bool TryCritScope::locked() const {
  LICODE_CS_DEBUG_CODE(lock_was_called_ = true);
  return locked_;
}

void GlobalLockPod::Lock() {
#if !defined(WEBRTC_WIN) && (!defined(WEBRTC_MAC) || LICODE_USE_NATIVE_MUTEX_ON_MAC)
...
#if defined(WEBRTC_WIN)
    ::Sleep(0);
#elif defined(WEBRTC_MAC) && !LICODE_USE_NATIVE_MUTEX_ON_MAC
    sched_yield();
#else
...
void GlobalLockPod::Unlock() {
  ...
  LICODE_RTC_DCHECK_EQ(1, old_value) << "Unlock called without calling Lock first";
}
