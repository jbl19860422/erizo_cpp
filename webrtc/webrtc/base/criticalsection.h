...
#ifndef LICODE_WEBRTC_BASE_CRITICALSECTION_H_
#define LICODE_WEBRTC_BASE_CRITICALSECTION_H_
...
#define LICODE_USE_NATIVE_MUTEX_ON_MAC 0
#if defined(WEBRTC_MAC) && !LICODE_USE_NATIVE_MUTEX_ON_MAC
#include <dispatch/dispatch.h>
#endif

#define LICODE_CS_DEBUG_CHECKS LICODE_RTC_DCHECK_IS_ON

#if LICODE_CS_DEBUG_CHECKS
#define LICODE_CS_DEBUG_CODE(x) x
#else  // !LICODE_CS_DEBUG_CHECKS
#define LICODE_CS_DEBUG_CODE(x)
...
class LOCKABLE CriticalSection {
 ...
 private:
  // Use only for LICODE_RTC_DCHECKing.
...
#endif
  LICODE_CS_DEBUG_CODE(mutable PlatformThreadRef thread_);
  LICODE_CS_DEBUG_CODE(mutable int recursion_count_);
#endif
};
...
class TryCritScope {
 ...
 private:
  const CriticalSection* const cs_;
  const bool locked_;
  LICODE_CS_DEBUG_CODE(mutable bool lock_was_called_);
