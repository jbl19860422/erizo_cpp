...
PlatformThread::PlatformThread(ThreadRunFunction func,
    ...
    : run_function_(func),
      ...
      thread_(0) {
#endif  // defined(WEBRTC_WIN)
  LICODE_RTC_DCHECK(func);
  LICODE_RTC_DCHECK(name_.length() < 64);
}
...
void PlatformThread::Start() {
  LICODE_RTC_DCHECK(thread_checker_.CalledOnValidThread());
  LICODE_RTC_DCHECK(!thread_) << "Thread already started?";
#if defined(WEBRTC_WIN)
  stop_ = false;
  ...
  LICODE_RTC_CHECK(thread_) << "CreateThread failed";
  LICODE_RTC_DCHECK(thread_id_);
#else
  ThreadAttributes attr;
  // Set the stack stack size to 1M.
  pthread_attr_setstacksize(&attr, 1024 * 1024);
  LICODE_RTC_CHECK_EQ(0, pthread_create(&thread_, &attr, &StartThread, this));
#endif  // defined(WEBRTC_WIN)
}
...
void PlatformThread::Stop() {
  ...
  LICODE_RTC_CHECK(queued || GetLastError() == ERROR_GEN_FAILURE);
...
#else
  stop_event_.Set();
  LICODE_RTC_CHECK_EQ(0, pthread_join(thread_, nullptr));
  thread_ = 0;
#endif  // defined(WEBRTC_WIN)
}
