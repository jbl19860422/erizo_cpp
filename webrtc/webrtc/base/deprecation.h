...
#ifndef LICODE_WEBRTC_BASE_DEPRECATION_H_
#define LICODE_WEBRTC_BASE_DEPRECATION_H_

// Annotate the declarations of deprecated functions with this to cause a
// compiler warning when they're used. Like so:
//
//   LICODE_RTC_DEPRECATED std::pony PonyPlz(const std::pony_spec& ps);
//
...
//   LICODE_RTC_DEPRECATED inline std::pony PonyPlz(const std::pony_spec& ps) {
...
#define LICODE_RTC_DEPRECATED __declspec(deprecated)
#elif defined(__GNUC__)
#define LICODE_RTC_DEPRECATED __attribute__ ((__deprecated__))
#else
#define LICODE_RTC_DEPRECATED
#endif
