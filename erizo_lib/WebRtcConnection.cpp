...
WebRtcConnection::WebRtcConnection(std::shared_ptr<Worker> worker, std::shared_ptr<IOWorker> io_worker,
                                   ...
                                   const std::vector<erizo::ExtMap> ext_mappings, WebRtcConnectionEventListener *listener) : connection_id_{connection_id},
...
void WebRtcConnection::addMediaStream(std::shared_ptr<MediaStream> media_stream)
{
    asyncTask([media_stream](std::shared_ptr<WebRtcConnection> connection) {
        ...
        if (connection->media_streams_.size() >= 1)
        {
            connection->remote_sdp_->is_publisher_ = media_stream->isPublisher();
            connection->remote_sdp_->label_ = media_stream->getLabel();
            connection->local_sdp_->is_publisher_ = media_stream->isPublisher();
            connection->local_sdp_->label_ = media_stream->getLabel();
        }
    });
}
...
bool WebRtcConnection::setRemoteSdpInfo(std::shared_ptr<SdpInfo> sdp, std::string stream_id)
{
    asyncTask([sdp, stream_id](std::shared_ptr<WebRtcConnection> connection) {
        ELOG_ERROR("%s message: setting remote SDPInfo", connection->toLog());
...
{
    asyncTask([sdp, stream_id](std::shared_ptr<WebRtcConnection> connection) {
        ELOG_DEBUG("%s message: setting remote SDP, stream_id=%s", connection->toLog(), stream_id.c_str());
        if (!connection->sending_)
        {
            return;
        }
        if (connection->media_streams_.size() >= 1 && !connection->media_streams_[0]->isPublisher())
        {
            connection->local_sdp_->video_ssrc_ = connection->media_streams_[0]->getVideoSinkSSRC();
            connection->local_sdp_->audio_ssrc_ = connection->media_streams_[0]->getAudioSinkSSRC();
        }

        connection->remote_sdp_->initWithSdp(sdp, "");
        connection->processRemoteSdp(stream_id);
        connection->local_sdp_->mline_first_ = connection->remote_sdp_->mline_first_;
    });
    return true;
}
    ...
    if (stream != media_streams_.end())
    {
        ...
        (*stream)->asyncTask([weak_this, stream_id](const std::shared_ptr<MediaStream> &media_stream) {
            if (auto connection = weak_this.lock())
            {
                ...
                ELOG_INFO("%s message: setting remote SDP to stream, stream: %s", connection->toLog(), media_stream->getId());
...
bool WebRtcConnection::addRemoteCandidate(const std::string &mid, int mLineIndex, const std::string &sdp)
{
    asyncTask([mid, mLineIndex, sdp](std::shared_ptr<WebRtcConnection> connection) {
        ...
        ELOG_DEBUG("message: Adding remote Candidate, candidate: %s, mid: %s, sdpMLine: %d",
                   sdp.c_str(), mid.c_str(), mLineIndex);
        if (connection->video_transport_ == nullptr && connection->audio_transport_ == nullptr)
        {
            ELOG_WARN("message: addRemoteCandidate on NULL transport");
            return;
        }
        MediaType theType;
        std::string theMid;
        ...
        if (mLineIndex == -1)
        {
            ELOG_DEBUG("message: All candidates received");
            if (connection->video_transport_)
            {
                connection->video_transport_->getIceConnection()->setReceivedLastCandidate(true);
            }
            else if (connection->audio_transport_)
            {
                connection->audio_transport_->getIceConnection()->setReceivedLastCandidate(true);
            }
            return;
        }

        if ((!mid.compare("video")) || (mLineIndex == connection->remote_sdp_->videoSdpMLine))
        {
            theType = VIDEO_TYPE;
            theMid = "video";
        }
        ...
        SdpInfo tempSdp(connection->rtp_mappings_);
        std::string username = connection->remote_sdp_->getUsername(theType);
        std::string password = connection->remote_sdp_->getPassword(theType);
        tempSdp.setCredentials(username, password, OTHER);
        if (tempSdp.initWithSdp(sdp, theMid))
        {
            if (theType == VIDEO_TYPE || connection->bundle_)
            {
                connection->video_transport_->setRemoteCandidates(tempSdp.getCandidateInfos(), connection->bundle_);
            }
            else if (theType == AUDIO_TYPE)
            {
                connection->audio_transport_->setRemoteCandidates(tempSdp.getCandidateInfos(), connection->bundle_);
            }
            else
            {
                ELOG_ERROR("message: add remote candidate with no Media (video or audio), candidate: %s", sdp.c_str());
            }
        }

        for (uint8_t it = 0; it < tempSdp.getCandidateInfos().size(); it++)
        {
            connection->remote_sdp_->addCandidate(tempSdp.getCandidateInfos()[it]);
        }
    });
    return true;
}
...
void WebRtcConnection::updateState(TransportState state, Transport *transport)
{
    ...
    case TRANSPORT_STARTED:
        ...
        {
            if ((!remote_sdp_->hasAudio || (audio_transport_.get() != nullptr && audio_transport_->getTransportState() == TRANSPORT_STARTED)) &&
                (!remote_sdp_->hasVideo || (video_transport_.get() != nullptr && video_transport_->getTransportState() == TRANSPORT_STARTED)))
            {
    ...
    case TRANSPORT_GATHERED:
        ...
        {
            if ((!local_sdp_->hasAudio || (audio_transport_.get() != nullptr && audio_transport_->getTransportState() == TRANSPORT_GATHERED)) &&
                (!local_sdp_->hasVideo || (video_transport_.get() != nullptr && video_transport_->getTransportState() == TRANSPORT_GATHERED)))
            {
                // WebRTCConnection will be ready only when all channels are ready.
                if (!trickle_enabled_)
                {
    ...
    case TRANSPORT_READY:
        ...
        {
            if ((!remote_sdp_->hasAudio || (audio_transport_.get() != nullptr && audio_transport_->getTransportState() == TRANSPORT_READY)) &&
                (!remote_sdp_->hasVideo || (video_transport_.get() != nullptr && video_transport_->getTransportState() == TRANSPORT_READY)))
            {
                // WebRTCConnection will be ready only when all channels are ready.
                temp = CONN_READY;
                trackTransportInfo();
