...
MediaStream::MediaStream(std::shared_ptr<Worker> worker,
                         ...
                         bool is_publisher) : audio_enabled_{false}, video_enabled_{false},
...
int MediaStream::deliverAudioData_(std::shared_ptr<DataPacket> audio_packet, const std::string &stream_id)
{
    if (audio_enabled_)
    {
...
int MediaStream::deliverVideoData_(std::shared_ptr<DataPacket> video_packet, const std::string &stream_id)
{
    if (video_enabled_)
    {
...
int MediaStream::deliverFeedback_(std::shared_ptr<DataPacket> fb_packet, const std::string &stream_id)
{
...
void MediaStream::onTransportData(std::shared_ptr<DataPacket> incoming_packet, MediaType media_type)
{
    if ((audio_sink_ == nullptr && video_sink_ == nullptr && fb_sink_ == nullptr))
    {
        return;
    ...
    worker_->task([stream_ptr, packet] {
        if (!stream_ptr->pipeline_initialized_)
        {
            ELOG_ERROR("onTransportData Pipeline not initialized yet");
            ELOG_DEBUG("%s message: Pipeline not initialized yet.", stream_ptr->toLog());
            return;
        }
        ...
        ELOG_ERROR("onTransportData packet_type=%d", packet->type);
        if (stream_ptr->pipeline_)
        {
            stream_ptr->pipeline_->read(std::move(packet));
        }
...
void MediaStream::read(std::shared_ptr<DataPacket> packet)
{
    ...
    else if (chead->packettype == RTCP_Sender_PT || chead->packettype == RTCP_SDES_PT)
    { // Sender Report
