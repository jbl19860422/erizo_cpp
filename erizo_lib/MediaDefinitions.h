...
class FeedbackSink {
 public:
    ...
    int deliverFeedback(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") {
        return this->deliverFeedback_(data_packet, stream_id);
    }
 private:
    virtual int deliverFeedback_(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") = 0;
};

class FeedbackSource {
 protected:
    FeedbackSink* fb_sink_;
    boost::mutex fb_sink_mux_;
 public:
    FeedbackSource(): fb_sink_{nullptr} {}
    virtual ~FeedbackSource() {}
    void setFeedbackSink(FeedbackSink* sink) {
       boost::mutex::scoped_lock lock(fb_sink_mux_);
        fb_sink_ = sink;
    }
};
...
class MediaSink: public virtual Monitor {
 ...
 public:
    int deliverAudioData(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") {
        return this->deliverAudioData_(data_packet, stream_id);
    }
    int deliverVideoData(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") {
        return this->deliverVideoData_(data_packet, stream_id);
    }
 ...
 private:
    virtual int deliverAudioData_(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") = 0;
    virtual int deliverVideoData_(std::shared_ptr<DataPacket> data_packet, const std::string &stream_id = "") = 0;
