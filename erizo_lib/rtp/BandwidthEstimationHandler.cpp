...
using licode::webrtc::RemoteBitrateEstimatorAbsSendTime;
using licode::webrtc::RemoteBitrateEstimatorSingleStream;

DEFINE_LOGGER(BandwidthEstimationHandler, "rtp.BandwidthEstimationHandler");

static const uint32_t kTimeOffsetSwitchThreshold = 30;
static const uint32_t kMinBitRateAllowed = 200;
...
std::unique_ptr<RemoteBitrateEstimator> RemoteBitrateEstimatorPicker::pickEstimator(bool using_absolute_send_time,
                                                                                    licode::webrtc::Clock *const clock,
                                                                                    RemoteBitrateObserver *observer)
{
    std::unique_ptr<RemoteBitrateEstimator> rbe;
    if (using_absolute_send_time)
    {
        rbe.reset(new licode::webrtc::RemoteBitrateEstimatorAbsSendTime(observer, clock));
    }
    else
    {
        rbe.reset(new licode::webrtc::RemoteBitrateEstimatorSingleStream(observer, clock));
    }
    return rbe;
}

BandwidthEstimationHandler::BandwidthEstimationHandler(std::shared_ptr<RemoteBitrateEstimatorPicker> picker) : stream_{nullptr}, clock_{licode::webrtc::Clock::GetRealTimeClock()},
                                                                                                               picker_{picker},
...
{
    licode::rtc::LogMessage::SetLogToStderr(false);
}
...
{
    licode::webrtc::RTPExtensionType type = licode::webrtc::kRtpExtensionNone;
    for (uint8_t id = 0; id < 10; id++)
    {
        ...
        case SSRC_AUDIO_LEVEL:
            type = licode::webrtc::kRtpExtensionAudioLevel;
            break;
        case ABS_SEND_TIME:
            type = licode::webrtc::kRtpExtensionAbsoluteSendTime;
            break;
        case TOFFSET:
            type = licode::webrtc::kRtpExtensionTransmissionTimeOffset;
            break;
        case VIDEO_ORIENTATION:
            type = licode::webrtc::kRtpExtensionVideoRotation;
            break;
        case TRANSPORT_CC:
            type = licode::webrtc::kRtpExtensionTransportSequenceNumber;
            break;
        case PLAYBACK_TIME:
            type = licode::webrtc::kRtpExtensionPlayoutDelay;
            break;
        }
        if (type == licode::webrtc::kRtpExtensionNone)
        {
            continue;
        }
        if (is_video)
...
bool BandwidthEstimationHandler::parsePacket(std::shared_ptr<DataPacket> packet)
{
    ...
    licode::webrtc::RtpUtility::RtpHeaderParser rtp_parser(buffer, length);
                                                         ...
                                                         uint32_t bitrate)
{
    ...
    stats_->getNode()
        [stream_->getVideoSourceSSRC()]
            .insertStat("erizoBandwidth", CumulativeStat{last_send_bitrate_});
    sendREMBPacket();
}
