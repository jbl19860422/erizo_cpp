...
SenderBandwidthEstimationHandler::SenderBandwidthEstimationHandler(std::shared_ptr<Clock> the_clock) : stream_{nullptr}, bwe_listener_{nullptr}, clock_{the_clock}, initialized_{false}, enabled_{true},
            ...
            case RTCP_PS_Feedback_PT:
            {
                if (chead->getBlockCount() == RTCP_AFB)
                {
                    ...
                    if (!strncmp(uniqueId, "REMB", 4))
                    {
                        ...
                        ELOG_DEBUG("%s message: Updating Estimate with REMB, bitrate %lu, cappedBitrate %lu, maxBitrate %lu", stream_->toLog(),
                            bitrate, cappedBitrate, processor_->getMaxVideoBW());//这里打印比较多，注意
