...
class FecReceiverHandler: public OutboundHandler, public licode::webrtc::RtpData {
  DECLARE_LOGGER();

 public:
  FecReceiverHandler();

  void setFecReceiver(std::unique_ptr<licode::webrtc::UlpfecReceiver>&& fec_receiver);  // NOLINT
  ...
  int32_t OnReceivedPayloadData(const uint8_t* payloadData, size_t payloadSize,
                                const licode::webrtc::WebRtcRTPHeader* rtpHeader) override;
