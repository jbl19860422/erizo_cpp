...
RtpPaddingRemovalHandler::~RtpPaddingRemovalHandler()
{
  ELOG_DEBUG("***************RtpPaddingRemovalHandler release start, %s***********", stream_->toLog());
}
...
void RtpPaddingRemovalHandler::read(Context *ctx, std::shared_ptr<DataPacket> packet) {
  ELOG_DEBUG("***************RtpPaddingRemovalHandler read start, %s***********", stream_->toLog());
  ...
  if (!chead->isRtcp() && enabled_ && packet->type == VIDEO_PACKET) {
    ...
    if (!removePaddingBytes(packet, translator)) {
      ELOG_DEBUG("***************RtpPaddingRemovalHandler read end, %s***********", stream_->toLog());
      return;
    }
...
void RtpPaddingRemovalHandler::write(Context *ctx, std::shared_ptr<DataPacket> packet) {
  ELOG_DEBUG("***************RtpPaddingRemovalHandler write start, %s***********", stream_->toLog());
  ...
  if (!translator) {
    ...
    ctx->fireWrite(std::move(packet));
    ELOG_DEBUG("***************RtpPaddingRemovalHandler write end, %s***********", stream_->toLog());
    return;
  }
