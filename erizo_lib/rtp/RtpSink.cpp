...
namespace erizo {
  ...
  int RtpSink::deliverVideoData_(std::shared_ptr<DataPacket> video_packet, const std::string &stream_id) {
    this->queueData(video_packet->data, video_packet->length, VIDEO_PACKET);
    return 0;
  }

  int RtpSink::deliverAudioData_(std::shared_ptr<DataPacket> audio_packet, const std::string &stream_id) {
