...
void SdpInfo::setAddressTransMap(const std::unordered_map<std::string, std::string> &m)
{
    address_trans_map_ = m;
}
...
std::string SdpInfo::stringifyCandidate(const CandidateInfo &candidate)
{
    ...
    auto hostAddress = candidate.hostAddress;
    if (address_trans_map_.size() > 0)
    {
        if(address_trans_map_.find(hostAddress) != address_trans_map_.end())
        {
            hostAddress = address_trans_map_[hostAddress];
        }
    }
    sdp << "a=candidate:" << candidate.foundation << " " << candidate.componentId
        << " " << candidate.netProtocol << " " << candidate.priority << " "
        << hostAddress << " " << candidate.hostPort << " typ "
        << hostType_str;
...
std::string SdpInfo::getSdp()
{
    ...
    if (mline_first_ == 1 || mline_first_ < 0)
    {
        if (printedAudio && this->hasAudio)
        {
            ...
            sdp << "a=end-of-candidates" << endl;
            if (!is_publisher_)
            {
                sdp << "a=ssrc:" << audio_ssrc_ << " cname:"
                    << "erizo" << endl;
                sdp << "a=ssrc:" << audio_ssrc_ << " msid:" << label_ << " "
                    << "audio" << endl;
                sdp << "a=ssrc:" << audio_ssrc_ << " mslabel:" << label_ << endl;
                sdp << "a=ssrc:" << audio_ssrc_ << " label:"
                    << "audio" << endl;
            }
        ...
        if (printedVideo && this->hasVideo)
        {
            ...
            if (!is_publisher_)
            {
                sdp << "a=ssrc:" << video_ssrc_ << " cname:"
                    << "erizo" << endl;
                sdp << "a=ssrc:" << video_ssrc_ << " msid:" << label_ << " "
                    << "video" << endl;
                sdp << "a=ssrc:" << video_ssrc_ << " mslabel:" << label_ << endl;
                sdp << "a=ssrc:" << video_ssrc_ << " label:"
                    << "video" << endl;
            }
...
bool SdpInfo::processSdp(const std::string &sdp, const std::string &media)
{
    ...
    while (std::getline(iss, line))
    {
        ...
        size_t isCname = line.find(cname);
        size_t isSsrc = line.find(ssrc);
        ...
        if (isCname != std::string::npos && isSsrc != std::string::npos)
        {
            if (mtype == AUDIO_TYPE)
            {
                ...
                if (parts.size() >= 4)
                {
                    uint32_t ssrc = strtoul(parts[1].c_str(), nullptr, 10);
                    audio_ssrc_map[label_] = ssrc;
                }
            }
            else if (mtype == VIDEO_TYPE)
            {
                ...
                {
                    uint32_t ssrc = strtoul(parts[1].c_str(), nullptr, 10);
                    video_ssrc_map[label_].push_back(ssrc);
                }
            }
        }
    } // sdp lines loop
...
void SdpInfo::getSSRC(uint32_t &video_ssrc, uint32_t &audio_ssrc)
{
    if (is_publisher_)
    {
        auto audio_it = audio_ssrc_map.find(label_);
        if (audio_it != audio_ssrc_map.end())
        {
            audio_ssrc = audio_it->second;
        }

        auto video_it = video_ssrc_map.find(label_);
        if (video_it != video_ssrc_map.end())
        {
            std::vector<uint32_t> video_ssrcs = video_it->second;
            if (video_ssrcs.size() > 0)
                video_ssrc = video_ssrcs[0];
        }
    }
    else
    {
        video_ssrc = video_ssrc_;
        audio_ssrc = audio_ssrc_;
    }
}
...
bool SdpInfo::postProcessInfo()
{
    ...
    for (const RtpMap &internal_map : internalPayloadVector_)
    {
        for (const std::pair<const unsigned int, RtpMap> &parsed_pair : payload_parsed_map_)
        {
            ...
            if (parsed_map.encoding_name == "rtx")
            { // we'll process those later when we have the pt maps
