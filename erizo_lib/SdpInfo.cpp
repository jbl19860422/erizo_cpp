  ...
  std::string SdpInfo::getSdp() {
    ...
    if(mline_first_ == 1 || mline_first_ < 0)
    {
      if (printedAudio && this->hasAudio) {
        ...
        if(!is_publisher_){
          sdp <<"a=ssrc:"<<audio_ssrc_<< " cname:" << "erizo"<<endl;
          sdp <<"a=ssrc:"<<audio_ssrc_<< " msid:"  <<  label_<<" "<<"audio"<<endl;
          sdp <<"a=ssrc:"<<audio_ssrc_<< " mslabel:" << label_<<endl;
          sdp <<"a=ssrc:"<<audio_ssrc_<< " label:" << "audio"<<endl;
        }
      ...
      if (printedVideo && this->hasVideo) {
        ...
        if(!is_publisher_)
        {
          sdp <<"a=ssrc:"<<video_ssrc_<< " cname:" << "erizo"<<endl;
          sdp <<"a=ssrc:"<<video_ssrc_<< " msid:"  << label_ << " "<< "video"<<endl;
          sdp <<"a=ssrc:"<<video_ssrc_<< " mslabel:" << label_<<endl;
          sdp <<"a=ssrc:"<<video_ssrc_<< " label:" << "video"<<endl;
        }
