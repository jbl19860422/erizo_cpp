...
DEFINE_LOGGER(OpusAudioDecoder, "media.codecs.OpusAudioDecoder");
...
int OpusAudioDecoder::initDecoder(int sample_rate, int channels) {
  if(initialized_) {
    return 0;
  }
  int error = 0;
...
int OpusAudioDecoder::decodeAudio(unsigned char* inBuff, int inBuffLen, unsigned char* outBuff, int outBuffLen, int* gotFrame)
{
  if(!initialized_) {
    return -1;
  }
...
int OpusAudioDecoder::closeDecoder() {
  if(opus_decoder_) {
